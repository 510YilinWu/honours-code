...
def update_properties_dict(file_path, search_terms):
    properties_dict.update({
        "ComponentsIndex": GetComponentsIndex(file_path, search_terms),
...
update_properties_dict(file_path, search_terms)
...
extractedData = initialize_extracted_data(properties_dict)
...
def populate_all_data(extractedData, properties_dict, file_path):
    for func in [populate_model_outputs, populate_joints, populate_segments, populate_trajectories]:
        func(extractedData, properties_dict, file_path)

populate_all_data(extractedData, properties_dict, file_path)
...
def save_extracted_data(extractedData, file_path):
    """
    Saves the extractedData dictionary to a Python file.
    Args:
...
# Call the function to save the extracted data
save_extracted_data(extractedData, file_path)
