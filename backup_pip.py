...
file_paths = filter_files_by_hand(tBBT_files, hand="left")
...
def write_indices_to_csv(right_indices, left_indices, file_save_path):
    csv_file_path = os.path.join(file_save_path, "final_selected_indices.csv")
    with open(csv_file_path, mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["Subject Name", "Hand", "File Path", "Selected Index"])
        
        # Write right hand indices
        for file_path, selected_indices in right_indices.items():
            subject_name = os.path.basename(file_path).split('_')[0]
            for index in selected_indices:
                writer.writerow([subject_name, "Right", file_path, index])
        
        # Write left hand indices
        for file_path, selected_indices in left_indices.items():
            subject_name = os.path.basename(file_path).split('_')[0]
            for index in selected_indices:
                writer.writerow([subject_name, "Left", file_path, index])
    
    print(f"✅ Indices written to {csv_file_path}")