...
def save_indices_to_single_csv(right_indices, left_indices, DataProcess_folder,iBBT_files):
    combined_file_path = os.path.join(DataProcess_folder, f"{os.path.basename(iBBT_files[0])[:-6]}_ReachStartAndEnd.csv")
...
def process_single_subject(date, traj_folder, Box_Traj_folder, figure_folder, DataProcess_folder, 
    ...
    if len(iBBT_files) != 8:
        raise ValueError(f"Expected 8 iBBT files, but found {len(iBBT_files)}")
    ...
    # --- CREATE SAVE PATHS ---
    plot_save_path = os.path.join(figure_folder, os.path.basename(iBBT_files[0])[:-6])
    os.makedirs(plot_save_path, exist_ok=True)

    # --- PROCESS TRAJECTORY DATA ---
    processed_single_subject_data = {}
    for hand in ["right", "left"]:
        file_paths = filter_files_by_hand(iBBT_files, hand=hand)
...
def process_all_dates_separate(All_dates, Traj_folder, Box_Traj_folder, Figure_folder, DataProcess_folder, 
    ...
    for date in All_dates:
        ...
        subject_filename = f"{date.replace('/', '_')}_iBBT_processed_data.pkl"
...
def save_results(results, DataProcess_folder, filename=None):
    ...
    Args:
        ...
        DataProcess_folder (str): Path to the data processing folder.
        filename (str, optional): Name of the file to save the results. Defaults to "iBBT_processed_results.pkl".

    Returns:
        None
    """
    if filename is None:
        filename = "iBBT_processed_results.pkl"  # Set default filename if not provided
